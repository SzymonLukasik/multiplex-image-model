...
# ---------------------------------------------------------------------------
# Learnable Mask Token (LMT) variants (non-equivariant)
# ---------------------------------------------------------------------------


class LMTMultiplexImageEncoder(MultiplexImageEncoder):
    """MultiplexImageEncoder with learnable mask token inserted after MA encoder.

    The mask token replaces feature-space patches that correspond to spatially
    masked input patches. Two token shapes are supported:

    - ``mask_token_shape='point'``: token of shape ``(1, E_ma, 1, 1)`` --
      a single 16-dim vector broadcast to every position in every masked patch.
      16 parameters.
    - ``mask_token_shape='patch'``: token of shape
      ``(1, E_ma, feat_patch, feat_patch)`` -- a full spatial pattern (e.g. 4x4)
      tiled once per masked patch. E_ma * feat_patch**2 parameters (256 for
      E_ma=16, feat_patch=4).

    The patch variant is only available in the non-equivariant model because
    a spatially-structured token would break rotation/flip equivariance.
    """
    ...
    def __init__(
            ...
            pm_embedding_dims: List[int],
            use_hyperkernel_act: bool = True,
            mask_token_shape: Literal['point', 'patch'] = 'point',
            mask_token_feat_patch: int = 4,
        ...
        super().__init__(
            ...
            hyperkernel_config=hyperkernel_config,
            pm_layers_blocks=pm_layers_blocks,
            pm_embedding_dims=pm_embedding_dims,
            use_hyperkernel_act=use_hyperkernel_act,
        )
        E_ma = ma_embedding_dims[-1] if len(ma_embedding_dims) > 0 else 1
        self.mask_token_shape = mask_token_shape
        self.mask_token_feat_patch = mask_token_feat_patch

        if mask_token_shape == 'point':
            self.mask_token = nn.Parameter(
                torch.randn(1, E_ma, 1, 1) * mask_token_init_std
            )
        elif mask_token_shape == 'patch':
            self.mask_token = nn.Parameter(
                torch.randn(1, E_ma, mask_token_feat_patch, mask_token_feat_patch)
                * mask_token_init_std
            )
        else:
            raise ValueError(
                f"mask_token_shape must be 'point' or 'patch', got {mask_token_shape!r}"
            )

    def _build_token_canvas(self, B_flat: int, H_ma: int, W_ma: int) -> torch.Tensor:
        """Build a feature-map-sized canvas filled with the mask token.

        For 'point' shape: broadcast the (1, E, 1, 1) vector.
        For 'patch' shape: tile the (1, E, p, p) pattern to cover (H_ma, W_ma).
        """
        if self.mask_token_shape == 'point':
            return self.mask_token.expand(B_flat, -1, H_ma, W_ma)

        # 'patch': tile the pattern and crop to actual feature map size.
        p = self.mask_token_feat_patch
        h_reps = -(-H_ma // p)
        w_reps = -(-W_ma // p)
        canvas = self.mask_token.repeat(1, 1, h_reps, w_reps)
        canvas = canvas[:, :, :H_ma, :W_ma]
        return canvas.expand(B_flat, -1, -1, -1)
    ...
    def forward(
            ...
            apply_spatial_mask: bool = False,
            input_patch_mask: Optional[torch.Tensor] = None,
            feat_mask_dilation: int = 0,
            return_features: bool = False,
    ) -> Dict:
        outputs = {}
        features = []
        ...
        # Spatial masking in feature space with learnable mask token
        if apply_spatial_mask and (spatial_mask_ratio > 0 or input_patch_mask is not None):
            feat_patch = mask_patch_size // 2  # non-equiv MA stem is stride 2
            h_patches = -(-H_ma // feat_patch)
            w_patches = -(-W_ma // feat_patch)

            if input_patch_mask is not None:
                patch_mask = input_patch_mask.to(device=x.device)
                assert patch_mask.shape == (B * C, 1, h_patches, w_patches), (
                    f"Expected patch_mask shape {(B*C, 1, h_patches, w_patches)}, "
                    f"got {tuple(patch_mask.shape)}"
                )
            else:
                patch_mask = (
                    torch.rand(B * C, 1, h_patches, w_patches, device=x.device)
                    < spatial_mask_ratio
                )

            mask = (
                patch_mask.repeat_interleave(feat_patch, dim=2)
                          .repeat_interleave(feat_patch, dim=3)
            )
            mask = mask[:, :, :H_ma, :W_ma]

            if feat_mask_dilation > 0:
                k = 2 * feat_mask_dilation + 1
                mask_f = F.max_pool2d(
                    mask.float(),
                    kernel_size=k, stride=1, padding=feat_mask_dilation,
                )
                mask = mask_f > 0.5

            # NOTE: for the 'patch' token shape, feat_patch MUST equal
            # mask_token_feat_patch so that the tiled token aligns 1-to-1 with
            # masked patches.
            if self.mask_token_shape == 'patch':
                assert feat_patch == self.mask_token_feat_patch, (
                    f"mask_patch_size/2 ({feat_patch}) must equal "
                    f"mask_token_feat_patch ({self.mask_token_feat_patch}) "
                    f"for 'patch' token shape."
                )

            token_canvas = self._build_token_canvas(B * C, H_ma, W_ma)
            x = torch.where(mask, token_canvas, x)
...
class LMTMultiplexAutoencoder(MultiplexAutoencoder):
    """MultiplexAutoencoder with LMTMultiplexImageEncoder.

    Accepts two extra encoder_config keys:
    - ``mask_token_shape``: 'point' (default) or 'patch'.
    - ``mask_token_feat_patch``: int, size of the spatial token when
      ``mask_token_shape='patch'`` (default 4).
    """
    ...
    def __init__(
        ...
        self.encoder = LMTMultiplexImageEncoder(
            num_channels=num_channels,
            **encoder_config,
        )

        scaling_factor = 2 ** len(
            encoder_config['ma_layers_blocks'] + encoder_config['pm_layers_blocks']
        )
        self.decoder = MultiplexImageDecoder(
            input_embedding_dim=self.latent_dim,
            scaling_factor=scaling_factor,
    ...
    def encode(
        ...
        encoding_output = self.encoder(
            ...
            input_patch_mask=input_patch_mask,
            feat_mask_dilation=feat_mask_dilation,
