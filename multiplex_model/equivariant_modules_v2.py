...
class EquivariantMultiplexImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        # 2*max_freq. Setting the gspace cap to 2*max_freq avoids
        # InsufficientIrrepsException at construction time.
        self._gspace = escnn.gspaces.flipRot2dOnR2(
            N=-1, maximum_frequency=2 * maximum_frequency
        )

        # channel-agnostic part — returns a BL-regular GeometricTensor
        ...
        self.hyperkernel = EquivariantHyperkernel(
            ...
            output_fields=K_out,
            gspace=self._gspace,
            bl_repr=self.marker_agnostic_encoder.bl_repr,
    ...
    def forward(self, x: torch.Tensor, encoded_indices: torch.Tensor, return_features: bool = False) -> Dict:
        ...
        if return_features:
            features += x['features']
        x = x['output']  # GeometricTensor of shape (B*C, K_in*R, H_ma, W_ma)

        t = x.tensor
        _, CR, H_ma, W_ma = t.shape  # CR = K_in * R (fields×repr contiguous)
        ...
        x = self.hyperkernel(t, encoded_indices)   # GeometricTensor [B, K_out*R, H_ma, W_ma]
        x = self.act(x)
