...
from multiplex_model.modules_lmt import LMTMultiplexAutoencoder
from multiplex_model.run_utils import build_run_name_suffix, SLURM_JOB_ID


# ---------------------------------------------------------------------------
# Spatial masking (no-leak variant): zero input patches + pass mask to model
# ---------------------------------------------------------------------------

def make_zero_masked_input(img, patch_size, ratio):
    """Sample a per-channel random patch mask, zero out masked input patches.

    Returns:
        masked_img: (B, C, H, W) with masked patches set to 0
        patch_mask: (B*C, 1, h_patches, w_patches) bool mask to pass to the model.
                    Shape matches the feature-space patch grid (stride-2 MA, so
                    h_patches = ceil(H/patch_size) = ceil((H/2) / (patch_size/2))).
    """
    B, C, H, W = img.shape
    h_patches = -(-H // patch_size)
    w_patches = -(-W // patch_size)
    patch_mask = torch.rand(
        B * C, 1, h_patches, w_patches, device=img.device,
    ) < ratio
    pixel_mask = (
        patch_mask.repeat_interleave(patch_size, dim=2)
                  .repeat_interleave(patch_size, dim=3)
    )
    pixel_mask = pixel_mask[:, :, :H, :W].reshape(B, C, H, W)
    masked_img = torch.where(pixel_mask, torch.zeros_like(img), img)
    return masked_img, patch_mask
...
def train_masked(
    ...
    """Train a masked autoencoder with DDP support (LMT no-leak variant).

    Spatial masking is applied in BOTH input space (zero out patches) AND
    feature space (replace with a learnable mask token, optionally dilated
    by ``feat_mask_dilation`` feature pixels to absorb MA receptive-field leak).
    Set ``apply_spatial_mask=False`` to disable feature-space masking entirely
    (input is still zeroed).
    """
    model.train()
    scaler = GradScaler()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (img, channel_ids, panel_idx, img_path) in enumerate(train_dataloader):
            ...
            # No-leak masking: zero input patches AND pass the same mask to the
            # model for token replacement (with receptive-field dilation).
            masked_img, input_patch_mask = make_zero_masked_input(
                masked_img, mask_patch_size, spatial_masking_ratio,
            )

            with autocast(device_type='cuda', dtype=torch.bfloat16):
                output = model(
                    masked_img, active_channel_ids, channel_ids,
                    spatial_mask_ratio=spatial_masking_ratio,
                    mask_patch_size=mask_patch_size,
                    apply_spatial_mask=apply_spatial_mask,
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0:
                ...
                # Monitor learnable mask token
                raw_model = model.module if isinstance(model, DDP) else model
                run['train/mask_token_norm'].append(
                    raw_model.encoder.mask_token.data.norm().item(), step=step
                )
...
if __name__ == '__main__':
    ...
    if config["model_type"] == "EquivariantConvnext":
        from multiplex_model.equivariant_modules_lmt import LMTEquivariantMultiplexAutoencoder
        model = LMTEquivariantMultiplexAutoencoder(**model_config).to(device)
    elif config["model_type"] == "Convnext":
        model = LMTMultiplexAutoencoder(**model_config).to(device)
    else:
    ...
    if config.get('from_checkpoint'):
        ...
        # strict=False allows loading old checkpoints that lack mask_token.
        missing, unexpected = model.module.load_state_dict(checkpoint['model_state_dict'], strict=False)
        if is_main_process() and missing:
            print(f'Missing keys in checkpoint (will use random init): {missing}')
    ...
    feat_mask_dilation = config.get('feat_mask_dilation', 0)
    apply_spatial_mask = config.get('apply_spatial_mask', True)
